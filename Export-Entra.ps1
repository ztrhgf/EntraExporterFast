function Export-Entra {
    ...
    Specifies the schema to use for the export. If not specified, the default schema will be used (generated by Get-EEDefaultSchema).

    .PARAMETER ThrottleLimit
    Specifies the maximum number of concurrent batch requests when running the export (PowerShell Core only). 
    ...
    function _executeBatchRequests {
        ...
        if ($batchRequestStableApi.Value.Count -gt 0) {
            ...
            $batchResults = Invoke-GraphBatchRequest -batchRequest $batchRequestStableApi.Value -ThrottleLimit $ThrottleLimit -separateErrors -ErrorAction SilentlyContinue -ErrorVariable requestErrors -WarningAction SilentlyContinue
        ...
        if ($batchRequestBetaApi.Value.Count -gt 0) {
            ...
            $batchResults = Invoke-GraphBatchRequest -batchRequest $batchRequestBetaApi.Value -graphVersion beta -ThrottleLimit $ThrottleLimit -separateErrors -ErrorAction SilentlyContinue -ErrorVariable requestErrors -WarningAction SilentlyContinue
